...
Step 43 完成了知识卡片数据集。Step 44 已完成最小索引和召回。Step 45 已把检索结果接入 `orchestrator -> dialogue-service`，但还没有做重排。
...
当前仓库在 Step 45 的真实路径是：

1. `apps/orchestrator` 调用 `rag-service`
2. 将返回的 `knowledge_cards` 写入对话请求 `metadata`
3. `services/dialogue-service` 在 prompt 中消费这些卡片
4. 若模型没有给出合法 `knowledge_refs`，服务端自动注入首张卡片的 `source_id`
5. 回复文本还会补一条与主卡片一致的建议句或追问句，确保“引用了知识，也真的用了知识”
...
Step 46 在检索层新增了高风险护栏，规则是：

- 当 `risk_level=high` 时，先只保留 `handoff_support` 和后续可扩展的 `safety_support`。
- 此时不再执行普通 `stage` 过滤，避免 `assess` 或 `intervene` 把 handoff 卡片错误排除。
- `filters_applied` 必须显式记录：
  - `risk_guardrail:high_only_safe_categories`
  - `stage:bypassed_for_high_risk_guardrail`
...
Step 45 当前已经验收“对话服务引用 `source_id` 并落到对应知识内容”，Step 46 已补上高风险检索护栏，避免普通卡片在高风险情境里混入。
