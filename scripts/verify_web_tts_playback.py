#!/usr/bin/env python3
"""Verify frontend TTS playback and subtitle sync against live gateway/orchestrator/dialogue/tts."""
...
def main() -> None:
    ...
    try:
        ...
        if result.returncode != 0:
            raise RuntimeError(
                "web tts playback harness failed: "
        ...
        audio_url = after_start["audioSrc"]

        if after_start["ttsPlaybackState"] != "playing":
            raise RuntimeError("frontend did not enter playing state")
        if after_end["ttsPlaybackState"] != "completed":
            raise RuntimeError("frontend did not enter completed playback state")
        if not audio_url:
            raise RuntimeError("frontend did not expose synthesized audio url")

        opener = urllib.request.build_opener(urllib.request.ProxyHandler({}))
        with opener.open(audio_url, timeout=10) as response:
            audio_body = response.read()
            if response.status != 200:
                raise RuntimeError("synthesized audio url is not reachable")
            if not audio_body:
                raise RuntimeError("synthesized audio response is empty")

        print(
            json.dumps(
                {
                    "session_id": after_end["sessionId"],
                    "assistant_reply": after_end["assistantReply"],
                    "audio_url": audio_url,
                    "playback_state": after_end["ttsPlaybackState"],
                    "voice_id": after_end["avatarVoice"],
                    "duration": after_end["avatarDuration"],
                },
                ensure_ascii=False,
                indent=2,
            )
