#!/usr/bin/env python3
"""Verify the static avatar baseline switches between idle and speaking in the live web flow."""
...
def main() -> None:
    ...
    try:
        ...
        if result.returncode != 0:
            raise RuntimeError(
                "web avatar baseline harness failed: "
        ...
        if before_create["avatarVisualState"] != "idle":
            raise RuntimeError("avatar baseline did not start in idle state")
        if after_start["avatarVisualState"] != "speaking":
            raise RuntimeError("avatar baseline did not switch to speaking state")
        if after_end["avatarVisualState"] != "idle":
            raise RuntimeError("avatar baseline did not return to idle state")

        print(
            json.dumps(
                {
                    "session_id": after_end["sessionId"],
                    "before_state": before_create["avatarCharacterState"],
                    "speaking_state": after_start["avatarCharacterState"],
                    "final_state": after_end["avatarCharacterState"],
                    "speaking_detail": after_start["avatarCharacterDetail"],
                    "final_detail": after_end["avatarCharacterDetail"],
                },
                ensure_ascii=False,
                indent=2,
            )
